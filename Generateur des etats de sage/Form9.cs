...
namespace Generateur_des_etats_de_sage
{
    public partial class Form9 : Form
    {
        ...
        public bool elementExist(string commercial)
        {
            bool exist = false;
            foreach (node reg in collection)
            {
                if (reg.commercial == commercial)
                {
                    exist = true;
                }
            }
        ...
        private void addElement(string commercial, decimal caht, DateTime d)
        {
            string month;
        ...
        {
            string sql = @"
                    SELECT sum(DO_TotalHT) as 'chiffre affaire',concat(CONCAT(CO_Nom,' '),CO_Prenom)
                    FROM F_DOCENTETE,F_COMPTET,F_COLLABORATEUR
                    where  DO_Domaine = 0
                    ...
                    group by concat(CONCAT(CO_Nom,' '),CO_Prenom)
                 ";
